...
//read all
    ...
    getHist_id: (bookid, result) => { 
     ...
     RealgetHist_id: (bookid, result) => { 
        return new Promise((resolve, reject) => {
            //SELECT lb.*,ct.nama_kategori FROM library as lb inner join category as ct on lb.id_kategori = ct.id_category WHERE id = ?
            konaksi.query(`SELECT history.*,library.nama_buku,library.foto_sampul, user.fullname,user.telepon,user.email,user.status,user.alamat,user.background FROM history left join library ON library.id_library = history.id_buku LEFT JOIN user ON history.id_peminjam = user.id_user WHERE id = ? ORDER BY history.tanggal_pinjam DESC `, bookid, (err, result) => {
                // `SELECT * FROM library WHERE id = ?`
                if(!err){
                    resolve(result)
                }else{
...
//read all history
    getheHist: (result) => { 
        return new Promise((resolve, reject) => {
            //SELECT lb.*,ct.nama_kategori FROM library as lb inner join category as ct on lb.id_kategori = ct.id_category
            konaksi.query(`SELECT history.*,library.nama_buku,library.foto_sampul,user.fullname,user.alamat FROM history INNER JOIN library ON history.id_buku = library.id_library INNER JOIN user ON history.id_peminjam = user.id_user ORDER BY history.tanggal_pinjam DESC`, (err, result) => {
                if(!err){
                    resolve(result)
                }else{
                    reject(new Error(err))
